...
def isbig(text_char):
    return (ord(text_char) > 64 and ord(text_char) < 91)
    
def istiny(text_char):
    return (ord(text_char) > 96 and ord(text_char) < 123)

text = text.replace('\n', '')

print("\n")
for i in range(0, len(text) - 5):
    if istiny(text[i]) and isbig(text[i+1]) and isbig(text[i+2]) and isbig(text[i+3]) and istiny(text[i+4]) and isbig(text[i+5]) and isbig(text[i+6]) and isbig(text[i+7]) and istiny(text[i+8]):
        print(text[i+4], end='')
